...
--    - CONSTANTS: Currency codes (ISO 4217), Country/State codes (ISO 3166), Work auth types
...
CREATE TABLE candidate_submission (
    ...
    -- Snapshot of key data at submission time (immutable)
    submitted_email VARCHAR(255),      -- Email used at time of submission
    submitted_phone VARCHAR(50),       -- Phone used at time of submission
...
-- Tracks current representation rights (who can represent which candidate to which employer)
-- Derived from candidate_submission but allows for admin overrides
CREATE TABLE candidate_representation (
    ...
    starts_at TIMESTAMP NOT NULL,      -- When representation began
    expires_at TIMESTAMP NOT NULL,     -- When it expires (typically 6 months)
    
    -- Reference to triggering submission
    ...
    override_reason TEXT,              -- Why admin changed representation
    created_by UUID,
    created_at TIMESTAMP DEFAULT NOW(),
    modified_by UUID,
    modified_at TIMESTAMP,
    ...
    INDEX idx_representation_candidate (person_id),
    INDEX idx_representation_recruiter (recruiter_person_id),
    INDEX idx_representation_employer (employer_tenant_id),
    INDEX idx_representation_expires (expires_at)
);

-- Duplicate detection conflicts requiring admin review
CREATE TABLE duplicate_conflict (
    ...
    INDEX idx_conflict_person1 (person_id_1),
    INDEX idx_conflict_person2 (person_id_2),
    INDEX idx_conflict_employer (employer_tenant_id),
...
-- Candidate searches
CREATE INDEX idx_candidate_tenant_availability ON candidate(person_id) 
WHERE EXISTS (SELECT 1 FROM person WHERE person.person_id = candidate.person_id);
...
-- Representation lookups
CREATE INDEX idx_representation_employer_active ON candidate_representation(employer_tenant_id, status, expires_at);
...
-- ALTER TABLE candidate_representation ENABLE ROW LEVEL SECURITY;
